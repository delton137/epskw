...
options = optimoptions(@lsqcurvefit,'MaxFunEvals',90000,'TolFun',1e-15,'MaxIter', 20000);

for k = 1:1
 
    fit_start = 1;         % Start of fit in inverse cm
...
%       params = [0.012,33.4/150   , 615/33.4 ,  ...  
%                 .003,33.4/100, 790/33.4,   ...
%                   0.1404 ,6.1407        ]; % Initial guesses for parameters (T) 18.4859
    params = [0.001,33.4/100   , 200/33.4 ,  ...  
                .00025,33.4/100, 620/33.4,   ...
                  0.01404 ,6.1407        ]; % Initial guesses for parameters (T) TTM3F
    else
        params = [A1(k-1), tau1(k-1), w01(k-1), A2(k-1), tau2(k-1),  w02(k-1), D1(k-1), Dtau(k-1) ]; % use values from previous fit  
    end
    ...
    DHO1 =  .5*abs(A1(k))*tau1(k)*freqs.*( (1 + tau1(k)^2*(freqs + w01(k)).^2 ).^(-1) + (1 + tau1(k)^2*(freqs - w01(k)).^2 ).^(-1) ); 
    DHO2 =  .5*abs(A2(k))*tau2(k)*freqs.*( (1 + tau2(k)^2*(freqs + w02(k)).^2 ).^(-1) + (1 + tau2(k)^2*(freqs - w02(k)).^2 ).^(-1) );
    
    fitcurve(k,:) = Debye + DHO1 + DHO2;
...
% plot1= 
plot(ffcm, chikw_fit,'k', ffcm,fitcurve(k,fit_window),'g',ffcm, chikw_fit-fitcurve(k,fit_window),'r',ffcm, Debye(fit_window),'b', ffcm, DHO1(fit_window),'o',ffcm, DHO2(fit_window),'y');
...
save -ascii TIP4P2005f_250_T_w_vs_k.dat matrix2save 	
